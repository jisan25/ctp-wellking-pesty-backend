...
<template>
    <div class="w-full max-w-[calc(100%-13rem)] h-16 border-b bg-white/50 backdrop-blur-xl fixed top-0 z-50">
        <div class="px-8 flex w-full h-full justify-between items-center">
            <a :href="frontendUrl" target="_blank" class="block px-4 py-2 hover:bg-primary my-4 bg-primary/20 group dark:hover:text-white transition-all ease-in-out duration-300 rounded-lg">
            ...
            <div class="relative cursor-pointer">
                ...
                <div @click="toggleDropdown" ref="dropdownButton" id="dropdownDefaultButton">
                    <div class="flex gap-2 items-center  px-2 rounded-md">
                        <img class="w-10 h-10 rounded-full object-cover" :src="authStore?.user?.image" alt="Rounded avatar">
                ...
                <div v-show="isDropdownOpen" ref="dropdown" id="dropdown" class="z-10 bg-white divide-y divide-gray-100 rounded-lg overflow-hidden shadow w-44 dark:bg-gray-700 absolute mt-2">
                    ...
                    <ul class="text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                        ...
                        <li>
                            <RouterLink :to="{name:'ProfileSetting'}" class="flex items-center gap-3 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
